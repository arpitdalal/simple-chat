...
Release builds that publish updater artifacts need `TAURI_SIGNING_PRIVATE_KEY` set to the private key contents or a file path — pubkey is in `src-tauri/tauri.conf.json`. Endpoint is GitHub Releases `latest.json` (publish once #1 CI lands). Linux updater packages are AppImage-only (`bundle.targets`).
